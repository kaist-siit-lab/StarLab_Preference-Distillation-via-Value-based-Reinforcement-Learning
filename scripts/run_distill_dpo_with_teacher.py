...
class DistillTrainer(Trainer):
    ...
    def concatenated_forward(self, model: nn.Module, batch: dict[str, Union[list, torch.LongTensor]]):
        ...
        if self.teacher_model is not None:
            with torch.no_grad():
                ...
                teacher_logits = teacher_outputs.logits[:, :-1, :]  # shape: [B, T, V]
    ...
    def get_batch_loss_metrics(
        ...
        if self.args.distillation_weight > 0:
            ...
            if self.teacher_model is not None:
                teacher_logits = model_output["teacher_logits"]
                base_log_pi = F.log_softmax(teacher_logits, dim=-1).detach()

                q_values = model_output["policy_rejected_probs"].log()  # or model_output["policy_rejected_logits"]
                ...
                q_values_ai = torch.gather(
                    q_values[:, :-1], -1, model_output["rejected_labels"][:, 1:].unsqueeze(-1)
                ).squeeze(-1)

                mask = (model_output["rejected_labels"][:, :-1] != -100)

                q_loss, q_log = qadapter_loss(q_values_ai=q_values_ai, values=values[:, :-1], mask=mask, gamma=gamma, beta=beta)

                losses = losses + self.args.adpa_weight * q_loss
        ...
        if self.args.adpa_weight > 0:
            ...
            if self.args.adpa_loss_type == "reverse_ce":
                ...
                adpa_loss = self.get_kl_loss(
                    policy_rejected_probs, teacher_probs, model_output["rejected_labels"]
                )
...
def main():
    ...
    teacher_model = None
    if training_args.distillation_weight > 0:
        teacher_model = AutoModelForCausalLM.from_pretrained(model_args.teacher_model_name_or_path, **model_kwargs)
        teacher_model.eval()
